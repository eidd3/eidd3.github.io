...
Enumero los puertos con el conjunto de scripts predetermindo de **nmap** y que detecte la version de cada uno.

![](https://eidd3.github.io/assets/img/DC01/puertos2.png)

Por algunos puertos abiertos como **kerberos**, **ldap**, **ADWS** y **RCP** supongo que puede ser un entorno de _Active Directory_, (aunque no todos los servicios son si o si propios de **AD**).

Con **crackmapexec** enumero informacion del sistema.
...
Agrego el nombre de dominio a mi archivo _/etc/hosts_.
...
Pero _smbmap_ no se logra conectar.

Si pruebo con otra herramienta como _smbclient_ logra enumerar algo.

![](https://eidd3.github.io/assets/img/DC01/smbclient.png)

Para ver si tengo algun permiso sobre los recursos uso **crackmapexec**, que como _smbmap_ te dice si hay permisos para leer.
...
Para este punto necesito al menos tener algun nombre de usuario para poder intentar algo en los servicios, y para esto se puede usar la herramienta **impacket-lookupsid**.
...
Por una malaconfiguracion, al usar esta herramienta con un null session se puede enumerar los usuarios del dominio.
...
Si intento un ataque **ASREProast** para ver si algún usuario es vulnerable y dumpea el hash para poder crackear no hay ningun resultado.

Asi que intento chekear si algún usuario reutiliza su nombre como contraseña.
...
Con **ldapdomaindump** veo los grupos y los miembros de estos, pero por ahora ningun usuario de esta lista pertenece al grupo **Remote Management Users**, por lo que no voy a poder acceder por **WinRM**.
...
Y existen 5 **Service Principal Name** asociados a cuentas de servicios. 
Pero no dumpea los hash por el error _Clock skew too great_, por lo que hay que sincronizar la hora del **DC** a la de mi maquina.
...
Guardo esos cinco nombres en un nuevo archivo y hago un _password spraying_ con **crackmapexec** para saber a quien pertenece la contraseña.
...
Los divido en 2 archivos para probar si hay algùn hash con el que poder hacer **Pass-The-Hash**.
